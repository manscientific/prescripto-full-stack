...
const MyProfile = () => {
  ...
  const { token = '', backendUrl = '', userData = null, setUserData = () => {}, loadUserProfileData = () => {} } = useContext(AppContext) || {}

  useEffect(() => {
    if (!userData) loadUserProfileData()
  }, [userData, loadUserProfileData]) // load on mount and when userData changes
  ...
  return (
    <div className='max-w-lg flex flex-col gap-2 text-sm pt-5'>
      {isEdit ? (
        <label htmlFor='image'>
          <div className='inline-block relative cursor-pointer'>
            <img
              ...
              src={image ? URL.createObjectURL(image) : (userData.image || '/placeholder-doctor.jpg')}
        ...
        <img className='w-36 rounded' src={userData.image || '/placeholder-doctor.jpg'} alt={userData.name || 'Profile'} />
      )}
      ...
      <div>
        ...
        <div className='grid grid-cols-[1fr_3fr] gap-y-2.5 mt-3 text-gray-600'>
          ...
          {isEdit ? (
            <select className='max-w-20 bg-gray-50' onChange={(e) => setUserData(prev => ({ ...prev, gender: e.target.value }))} value={userData.gender || 'Not Selected'}>
